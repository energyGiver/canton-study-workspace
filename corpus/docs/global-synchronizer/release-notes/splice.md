...
> Release notes and version history for Global Synchronizer software
...
* Validator App
  ...
  * Added a `type` parameter to validator config's `reward-sharing-config-by-party` option.
    ...
    [https://docs.canton.network/global-synchronizer/splice-fundamentals/reward-sharing#reward-sharing](https://docs.canton.network/global-synchronizer/splice-fundamentals/reward-sharing#reward-sharing)

    Example enabling external sharing automation for a party::

    ```canton.validator-apps.<validator>.reward-sharing-config-by-party = { theme={"theme":{"light":"github-light","dark":"github-dark"}}
      "<party-id>" = {
        type = "external"
...
* Validator
  ...
  * The `splice-postgres` Helm chart is deprecated and will not be supported after
    ...
    recommended. Follow the `migration guide <https://docs.canton.network/global-synchronizer/production-operations/validator-postgres-migration>`\_\_ to move the data
...
* Daml
  ...
  * Implement all Token Standard V2 APIs for Amulet, which adds the following new features to Amulet:
    ...
    * Greatly simplify the parsing of Amulet transaction history for wallets
      ...
      [V2.EventLog\_HoldingsChange](#type-splice-api-token-transfereventsv2-eventlogholdingschange-79855) events.
...
<Warning title="Important">
  ...
  [pg\_checksums documentation](https://www.postgresql.org/docs/14/app-pgchecksums.html) ).
...
* Deployment
  ...
  * Added JAVA\_TOOL\_OPTIONS to the docker compose validator deployment.
...
* Deployment
  ...
  > * Switch docker base images to [https://github.com/canton-network/canton-base-images](https://github.com/canton-network/canton-base-images) to reduce attack surface.
...
* UI
  ...
  > * offline\_access scope has been removed from the default requested scopes for OAuth tokens in the CN UIs. When a token expires, users will now be prompted to log in again instead of the UI refreshing the token.
...
* SV
  ...
  * The extra beneficiaries weight config has been fixed to accept integer values. The string values for weight have been deprecated and will be removed in future releases. It is recommended to fix the config as per this example, the previous config:
    ...
    * beneficiary: "BENEFICIARY\_1\_PARTY\_ID"
      weight: "1000"
...
<Warning>
  * This release includes a change to the database schema that will trigger a long database migration of the scan and validator app databases, resulting in increased downtime of SV nodes, and to a much lesser extent the validator nodes.
    ...
    * Make sure you the [temp\_file\_limit](https://www.postgresql.org/docs/current/runtime-config-resource.html#GUC-TEMP-FILE-LIMIT) Postgres parameter is set to a sufficiently high number. The actual usage is hard to predict, so we recommend setting it to the maximum value for the duration of the migration.
...
* Daml
  * Restructured the Daml code of AmuletRules\_BuyMemberTraffic to avoid an intermediate transfer to the DSO party before the amulets were burned. There is no change in the amount that gets burned or the rewards are issued, just a slight change in the transaction structure to accomplish this.
...
* SV app
  ...
  * The extra beneficiary config has been changed to specify weights in an ordered list instead of percentages. The weights are distributed in the order of the list until there is no weight remaining. Any remainder still goes to the SV operator party. This fixes two problems with the percentage-based beneficiary specification:
    ...
    * partyId: "BENEFICIARY\_1\_PARTY\_ID"
      percentage: 10.0
    * partyId: "BENEFICIARY\_2\_PARTY\_ID"
      percentage: 33.33
    ...
    * beneficiary: "BENEFICIARY\_1\_PARTY\_ID"
      weight: 1000
    * beneficiary: "BENEFICIARY\_2\_PARTY\_ID"
      weight: 3333
