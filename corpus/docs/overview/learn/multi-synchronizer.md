...
> How contracts are assigned to synchronizers and moved between them
...
For each input contract c\_i, denote by S\_i its assignation (at the submission time). A Synchronizer `S` is admissible for the transaction if:

* The reassignment of c\_i from S\_i to `S` is valid.
* The submitting participant is a reassigning participant for the submitting party for each c\_i.
