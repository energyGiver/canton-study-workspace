...
Canton uses a UTXO model, where the UTXOs are all the active contracts that implement the `Holding` interface. Active `Holding` contracts incur storage and compute cost on validator nodes, so wallet providers should keep the number of UTXOs per user low and prefer selecting small-amount `Holding` UTXOs as inputs to transfers.

See [Holding UTXO Management](/appdev/deep-dives/token-standard#holding-utxo-management) for the full guidance, including why low UTXO counts matter and how to set up `MergeDelegation` contracts during user onboarding so wallets can automatically merge small UTXOs on the user's behalf.
